...
class AdventureDecoderGUI:
    ...
    def detect_steam_path(self):
        ...
        try:
            ...
            user_dirs = [d for d in os.listdir(steam_base) 
                        if os.path.isdir(os.path.join(steam_base, d))]
            
            if user_dirs:
                user_id = user_dirs[0]
                save_path = os.path.join(steam_base, user_id, "462930", "remote", "game.sav")
    ...
    def create_widgets(self):
        ...
        ttk.Label(top_frame, text="Save File Path:").grid(row=0, column=0, sticky=tk.W, pady=5)
        ...
        self.output_text = scrolledtext.ScrolledText(output_frame, width=100, height=35, 
                                                      font=("Consolas", 9))
            ...
            for card_id in id_range:
                if card_id in decoded_data['cards']:
                    ...
                    if value > 0:
                        ...
                        if value > 1e6:
                            industry_lines.append(f"  [{card_id:2d}] {name:30s}: {value:.2e}")
                        else:
                            industry_lines.append(f"  [{card_id:2d}] {name:30s}: {value:,.0f}")
