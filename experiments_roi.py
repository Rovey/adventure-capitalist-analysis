...
def calculate_experiment_roi(experiment_name, experiment_data, current_scientists, current_production):
    """
...
def analyze_experiments(decoded_data, researched_experiments=None):
    ...
    if 'cards' in decoded_data:
        ...
        for card_id, name in resource_map.items():
            if card_id in decoded_data['cards']:
                current_production[name] = decoded_data['cards'][card_id].get('value', 0)
    ...
    for exp_name, exp_data in EXPERIMENTS.items():
        ...
        roi = calculate_experiment_roi(exp_name, exp_data, current_scientists, current_production)
        if roi:
    ...
    recommendations.sort(key=lambda x: (x['roi_score'], x['industry_priority']), reverse=True)
...
def format_experiment_recommendations(recommendations, current_scientists, top_n=15):
    ...
    if affordable:
        ...
        for i, rec in enumerate(affordable[:top_n], 1):
            mult_str = f"x{rec['multiplier']:,}" if isinstance(rec['multiplier'], (int, float)) else rec['multiplier']
            ...
            output.append(f"   Priority: {'⭐' * rec['priority']} ({rec['priority']}/10)")
    ...
    if unaffordable and len(affordable) < top_n:
        ...
        for i, rec in enumerate(unaffordable[:remaining], len(affordable) + 1):
            mult_str = f"x{rec['multiplier']:,}" if isinstance(rec['multiplier'], (int, float)) else rec['multiplier']
            output.append(f"{i}. {rec['name']} - Need {rec['cost'] - current_scientists} more Scientists")
            output.append(f"   Cost: {rec['cost']:,} | Effect: {mult_str} | Priority: {rec['priority']}/10")
            output.append("")
    ...
    for exp_type in ['INDUSTRY', 'STATE', 'TRIALS']:
        if exp_type in best_by_type:
            ...
            output.append(f"{exp_type:12} - {rec['name']:30} | {mult_str:12} | {rec['cost']:3} Scientists | {status}")
    
    # Strategy tip
    output.append(f"\n{'='*90}")
    output.append("💡 STRATEGY TIP")
    output.append(f"{'='*90}")
    output.append("1. INDUSTRY experiments (x99999, x9999, x999) are PERMANENT - highest priority!")
    output.append("2. When ROI is equal, recommendations prioritize your weakest industries")
    output.append("3. Focus on weakest industries (Medicine → Weapons → Ore → Land → Potato)")
    output.append("4. Button Auto-Clickers is extremely valuable - saves tons of time")
    output.append("5. TRIALS/STATE experiments are temporary - only buy when needed for missions")
